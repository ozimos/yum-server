...
class Controller {
    ...
    getAllRecords(req, res, next) {
        const { scopes, message: altMessage, statusCode = 200, options } = this.cloneResetConfig();
    ...
    deleteRecord(req, res, next) {
        const { scopes, message: altMessage, options = {} } = this.cloneResetConfig();
        return this.Model.scope(scopes)
            .destroy(
                this.mergeOptions(
                    {
            ...
            .then((result) => {
                if (result) {
                    return res.status(200).json({ message: altMessage || 'record was deleted' });
                }
                return res.status(404).json({ message: altMessage || 'record was not deleted' });
            })
            .catch((error) => next(error));
    }
}
