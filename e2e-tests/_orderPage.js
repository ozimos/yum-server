...
module.exports = {
  ...
  'It should not add meal to cart if meal has already been added to cart':
   (browser) => {
     browser
       .pause(3000)
  ...
  'It should navigate to cart and place order': (browser) => {
    browser
      ...
      .pause(1000)
      .setValue('#order-cart > div.responsive-table > table > tbody >' +
      'tr:nth-child(1) > td.quantity-cell > input', 3)
      .pause(2000)
      .click('#place-order')
      .waitForElementVisible('.Toastify__toast-body', 3000)
  ...
  'It should not remove meal from cart if only one meal is left': (browser) => {
    browser
      ...
      .click('body > .ReactModalPortal tr.slim:first-of-type >' +
