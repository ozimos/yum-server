...
export const cashTotal = (orders) =>
    orders ? orders.reduce((accum, order) => accum + subTotal(order.toJSON().Meals), 0) : 0;

export const uniqueUsers = (rows) => new Set(rows.map((row) => row.userId)).size;
...
export default class OrderController extends Controller {
    ...
    getTotalSales(req, res, next) {
        const { scopes: extraScopes = [], statusCode = 200 } = this.cloneResetConfig();
    ...
    getOrdersWithLinksByDate(req, res, next) {
        this.config = { scopes: [this.setDiscrimDate(req)] };
        return this.getOrdersWithLinks(req, res, next).catch((error) => next(error));
    }
