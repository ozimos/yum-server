...
class Form extends React.Component {
  ...
  validate() {
    ...
    const { password, confirmPassword } = formEl.elements;
    ...
    */
    if (formEl.checkValidity() === false) {
      for (let i = 0; i < formLength; i += 1) {
        ...
        */
        if (errorLabel && elem.nodeName.toLowerCase() !== 'button') {
          ...
          * The elem.validity.valid property indicates whether
          * the element qualifies its validation rules or no.
          * If it does not qualify, the elem.validationMessage
          * property will contain the localized validation error message.
          * We will show that message in our error container if the
          * element is invalid, and clear the previous message, if it is valid.
          */
      ...
      // Return 'false', as the formEl.checkValidity() method
      // said there are some invalid form inputs.
      return false;
    }
  ...
  * */
  submitHandler(event) {
    ...
    // If the call of the validate method was successful,
    // we can proceed with form submission. Otherwise we do nothing.
    if (this.validate()) {
  ...
  * */
  render() {
    ...
    // The form will have a refference in the component and
    // a submit handler set to the component's submitHandler
    const childWithProp = React.Children.map(this.props.children, child =>
      React.cloneElement(child, { onChange: this.changeHandler }));
