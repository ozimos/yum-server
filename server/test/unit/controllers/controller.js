...
describe("Controllers", () => {
  ...
  describe("getAllRecords()", () => {
    ...
    it("should return a list of rows if data is returned from database", () => {
      ...
      td.when(Table.findAndCountAll(td.matchers.anything())).thenResolve({
        count: 1,
        rows: expectedResponse
      });

      return controller
        .getAllRecords(req, scope)
    ...
    it("should return an error message if error occurs when accessing database", () => {
      ...
      const req = { query };
      td.when(Table.scope(scope)).thenReturn(Table);
      td.when(Table.findAndCountAll(td.matchers.anything())).thenReject(error);
      return controller
        .getAllRecords(req, scope)
  ...
  describe("getSingleRecord()", () => {
    ...
    it("should return a row if data is returned from database", () => {
      ...
      td.when(Table.findByPk(req.params.id, undefined)).thenResolve(
        expectedResponse
      );

      return controller
        .getSingleRecord(req, undefined)
    ...
    it("should return an error message if no data in database", () => {
      ...
      td.when(Table.findByPk(req.params.id, undefined)).thenResolve(null);
      return controller
        .getSingleRecord(req, undefined)
    ...
    it("should return an error message if error occurs when accessing database", () => {
      ...
      td.when(Table.findByPk(req.params.id, undefined)).thenReject(error);
      return controller
        .getSingleRecord(req, undefined)
        .catch(response => expect(response.message).to.equal(error.message));
    });
  ...
  describe("postRecord()", () => {
    it("should create a row", () => {
      ...
      return controller.postRecord(req).then(response => {
  ...
  describe("updateRecord()", () => {
    ...
    it("should update a row", () => {
      ...
      return controller
        .updateRecord(req)
        .then(response => expect(response.data).to.eql(req.body));
    });
